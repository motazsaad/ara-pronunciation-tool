...
phones = ["<", "b", "t", "^", "j", "H", "x", "d", "*", "r", "z", "s", "$", "S", "D", "T", "Z", "E", "g", "f", "q", "k",
          "l", "m", "n", "h", "w", "y", "aa", "uu0", "ii0", "a", "u0", "i0", "AA", "A", "u1", "i1", "<<", "bb", "tt",
          "^^", "jj", "HH", "xx", "dd", "**", "rr", "zz", "ss", "$$", "SS", "DD", "TT", "ZZ", "EE", "gg", "ff", "qq",
          "kk", "ll", "mm", "nn", "hh", "ww", "yy", "sil"]
...
types = {"phones": phones, "consonants": consonants, "vowels": vowels, "stops": stops, "stopsVoiced": stopsVoiced,
         "stopsVoiceless": stopsVoiceless, "fric": fric, "fricVoiced": fricVoiced, "fricVoiceless": fricVoiceless,
         "nasals": nasals, "trill": trill, "approx": approx, "pause": pause}


def diphones(target):  # Directory of the phonetised transcript
    lines = []
    diphoneResults = {}
    diphoneTypeResults = {}
    ...
    for line in lines:
        ...
        if len(line) > 1:
            for i in range(1, len(line)):
                ...
                cur = re.sub("U", "u", cur)
                cur = re.sub("I", "i", cur)
                next_char = re.sub("U", "u", next_char)
                next_char = re.sub("I", "i", next_char)
                if len(prev) > 1 and prev[1:] in vowels:
                    prev = prev[1:]

                if prev + '-' + cur in diphoneResults and not (
                            prev in vowels and cur in consonants and next_char in vowels):
    ...
    return diphoneResults, diphoneTypeResults


if __name__ == '__main__':
    diphoneResults, diphoneTypeResults = diphones(sys.argv[1])
    res = ''
