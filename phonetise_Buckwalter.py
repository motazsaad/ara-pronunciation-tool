...
def isFixedWord(word, results, orthography, pronunciations):
    ...
    # check if word is in the fixed word lookup table
    ...
    for word in utterance:
        ...
        if (not word in ['-', 'sil']):
            ...
            result = isFixedWord(word, result, word,
                                 pronunciations)  # Add fixed irregular pronuncations if possible
            ...
            for index in range(2, len(word) - 2):
                ...
                if (letter == u'l'):  # Lam is a consonant which requires special treatment
                    if ((not letter1 in diacritics and not letter1 in vowelMap) and letter2 in [
                        u'~']):  # Lam could be omitted in definite article (sun letters)
